 ...
 */
export function createPassportMcpServer(
  ...
  server.tool(
    "passport_anchor_task",
    "Anchor a task by issuing a competence receipt",
    {
  ...
  server.tool(
    "passport_close_task",
    "Close a task with status derived from error tranche",
    {
  ...
  server.tool(
    "passport_query_gate",
    "Query gate pass for an operator and domain",
    {
  ...
  server.tool(
    "passport_query_por",
    "Query live physical commodity Proof-of-Reserves (PoR), Merkle root, and audited lot inclusion proofs",
    {
  ...
  server.tool(
    "passport_get_regime_state",
    "Query Dual-State Governor regime (SOLID vs GHOST), Bayesian stress metrics, dynamic damping fees, and multi-commodity spot prices",
    {},
    async () => {
  ...
  server.tool(
    "passport_create_commodity_escrow",
    "Create a bilateral physical-commodity escrow, locking AngelCoin collateral against a vaulted lot",
    {
  ...
  server.tool(
    "passport_release_commodity_escrow",
    "Release a held commodity escrow to the seller after verified assay certification",
    {
      escrow_id: z.string().min(1),
  ...
  server.tool(
    "passport_refund_commodity_escrow",
    "Refund the buyer's collateral for a commodity escrow that has timed out",
    {
      escrow_id: z.string().min(1),
    },
    async (args) => {
  ...
  server.tool(
    "passport_get_sovereign_dividends",
    "Query macroeconomic Sovereign Dividend Waterfall & Anti-Extraction distributions across national treasury, community trusts, and mine workers",
    {
      limit: z.number().optional(),
    },
    async (args) => {
  ...
  server.tool(
    "passport_record_ore_intake",
    "Record verified raw doré gold intake from an artisanal miner at a field buying counter with 95% spot payout",
    {
  ...
  server.tool(
    "passport_list_artisanal_stations",
    "List active artisanal gold buying stations, district locations, bonded assayer stakes, and formalization metrics across the Sahel",
    {},
    async () => {
  ...
  server.tool(
    "passport_submit_quorum_signature",
    "Submit an Ed25519 signature from a sovereign state ministry (ML, BF, NE) for a 2-of-3 governance proposal",
    {
  ...
  server.tool(
    "passport_list_quorum_proposals",
    "List active and historical Trilateral Sovereign Governance (AES 2-of-3) proposals, voting progress, and dead-man heartbeats",
    {
      limit: z.number().optional(),
    },
    async (args) => {
  ...
  server.tool(
    "passport_record_smelting_telemetry",
    "Ingest automated smelting pour telemetry (gross weight, density specific gravity, inline XRF Au/Ag fineness) signed by industrial mine-gate furnace edge HSM",
    {
  ...
  server.tool(
    "passport_list_industrial_concessions",
    "List registered industrial gold mining concessions (Loulo-Gounkoto, Fekola, Essakane), statutory royalty rates, and extraction metrics",
    {},
    async () => {
  ...
  server.tool(
    "passport_register_industrialization_project",
    "Register a sovereign industrialization project funded by the 30% counter-cyclical stabilization treasury (Q142)",
    {
  ...
  server.tool(
    "passport_list_fund_projects",
    "List sovereign industrialization fund projects, stabilization balance, counter-cyclical solvency metrics, and deployed capital",
    {},
    async () => {
  ...
  server.tool(
    "passport_verify_fund_milestone",
    "Verify and release a stabilization fund milestone disbursement upon credible neutral verifier signature over drone/satellite imagery digest",
    {
 ...
 */
export async function startPassportMcpServer(): Promise<void> {
  const baseUrl = process.env.PASSPORT_BASE_URL;
  const apiKey = process.env.PASSPORT_API_KEY;

  if (!baseUrl || !apiKey) {
    throw new Error(
      "PASSPORT_BASE_URL and PASSPORT_API_KEY environment variables are required"
    );
  }
