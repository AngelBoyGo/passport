...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit(`reserves:industrial:bridge:${ip}`, 30, 60_000);
  if (!rate.allowed) {
  ...
  const runNumbers = Array.isArray(body.run_numbers || body.runNumbers)
    ? (body.run_numbers || body.runNumbers) as string[]
    : [];
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Missing required fields: run_numbers, target_batch_number, vault_id, custodian_name, refined_gross_grams, refined_fineness",
      },
      { status: 400 }
    );
  }
    ...
    return NextResponse.json(
      {
        ...
        refined_metrics: {
          runs_refined_count: result.runsRefinedCount,
