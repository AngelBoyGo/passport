    ...
    <div className="min-h-screen bg-[#080b12] text-slate-100">
      <div className="mx-auto flex max-w-[1500px] gap-6 px-4 py-5 lg:px-8">
        ...
        <main className="min-w-0 flex-1">
          <header className="flex flex-wrap items-end justify-between gap-4 border-b border-white/10 pb-5"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-300">{currentTab?.eyebrow}</p><h2 className="mt-2 text-3xl font-semibold tracking-tight">{currentTab?.label}</h2><p className="mt-1 text-sm text-slate-400">Live snapshot · {new Date(data.generatedAt).toLocaleTimeString()}</p></div><button onClick={() => load().catch((reason) => setError(String(reason)))} className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-slate-300 hover:bg-white/10">Refresh data</button></header>
          ...
          {tab === "trust-operations" && (
            <>
              <div className="mt-6 grid gap-3 sm:grid-cols-3"><Metric label="Issued passports" value={number.format(metrics.issuedAgents)} detail="Enrolled agents" tone="emerald" href="/leaderboard" /><Metric label="Evidence observed" value={number.format(metrics.evidence)} detail="Privacy-safe events" tone="sky" href="/leaderboard" /><Metric label="Receipts today" value={number.format(metrics.receiptsToday)} detail="Last 24 hours" tone="indigo" href="/admin/receipts" /></div>
              <div className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
                <Panel title="Enrollment pipeline" eyebrow="Identity"><div className="grid gap-3 sm:grid-cols-2"><Posture label="Enrolled agents" value={number.format(metrics.issuedAgents)} detail="Active passports" /><Posture label="Evidence events" value={number.format(metrics.evidence)} detail="Total ingested" /></div></Panel>
                <Panel title="Quick actions" eyebrow="Operations"><div className="space-y-3"><Link href="/enroll" className="block rounded-lg border border-white/10 bg-white/[0.02] p-4 text-sm text-slate-300 hover:bg-white/5">Enroll a new agent →</Link><Link href="/post-evidence" className="block rounded-lg border border-white/10 bg-white/[0.02] p-4 text-sm text-slate-300 hover:bg-white/5">Post evidence →</Link><Link href="/admin/webhooks" className="block rounded-lg border border-white/10 bg-white/[0.02] p-4 text-sm text-slate-300 hover:bg-white/5">Configure webhooks →</Link></div></Panel>
              </div>
            </>
          )}
          ...
          {tab === "economy" && (
            <>
              <div className="mt-6 grid gap-3 sm:grid-cols-3"><Metric label="Credits available" value={number.format(data.operator.credits)} detail={`${data.operator.tier} tier`} tone="indigo" href="/admin/api-keys" /><Metric label="Engagements" value={number.format(metrics.engagements)} detail="Active marketplace" tone="sky" href="/admin" /><Metric label="Slashing exposure" value={`$${(metrics.slashedCents / 100).toFixed(2)}`} detail={`${metrics.slashingEvents} events`} tone="amber" href="/admin" /></div>
              <div className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
                <Panel title="Account balance" eyebrow="Operator"><div className="grid gap-3 sm:grid-cols-2"><Posture label="Stake balance" value={`$${(data.operator.stakeBalanceCents / 100).toFixed(2)}`} detail="Minimum $50 escrow" /><Posture label="Account status" value={data.operator.accountStatus} detail={data.operator.accountStatus === "ACTIVE" ? "All operations permitted" : "Blocked — escrow insolvent"} /></div></Panel>
                <Panel title="Engagement lifecycle" eyebrow="Marketplace"><div className="space-y-2 text-sm text-slate-400"><p>Held: funds locked in escrow</p><p>Delivered: evidence received</p><p>Paid: escrow released to worker</p><p className="mt-2 text-xs text-slate-500">Total engagements: {number.format(metrics.engagements)}</p></div></Panel>
              </div>
            </>
          )}
          ...
          {tab === "reliability" && (
            <>
              <div className="mt-6 grid gap-3 sm:grid-cols-4"><Metric label="Health posture" value={data.health.overall} detail={data.health.overall === "operational" ? "All systems nominal" : "Some components degraded"} tone={data.health.overall === "operational" ? "emerald" : "amber"} href="#reliability" /><Metric label="Receipts issued" value={number.format(metrics.receipts)} detail="All time" tone="indigo" href="/admin/receipts" /><Metric label="Evidence observed" value={number.format(metrics.evidence)} detail="Privacy-safe" tone="sky" href="/leaderboard" /><Metric label="Issued passports" value={number.format(metrics.issuedAgents)} detail="Enrolled agents" tone="emerald" href="/leaderboard" /></div>
